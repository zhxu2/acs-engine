...
func FormatAzureProdFQDN(fqdnPrefix string, location string) string {
	...
	case azureUSSecCloud:
		FQDNFormat = AzureUSSecCloudSpec.EndpointConfig.ResourceManagerVMDNSSuffix
	default:
...
func GetCloudTargetEnv(location string) string {
	...
	case strings.HasPrefix(loc, "ussec"):
		return azureUSSecCloud
	default:
...
func getParameters(cs *api.ContainerService, isClassicMode bool, generatorCode string) (paramsMap, error) {
	...
	if properties.OrchestratorProfile.OrchestratorType == api.Kubernetes {
		...
		addValue(parametersMap, "vnetCniMultitenancyLinuxPluginsURL", cloudSpecConfig.KubernetesSpecConfig.VnetCNIMultitenancyLinuxPluginsDownloadURL)
